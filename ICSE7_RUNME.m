...
% Below is an advanced use of the plotter. On the first call, many things
% can be configured, such as the size of the figure, and the fontsize of
% all graphics object. A title is added, wavelength is converted to eV, a
% minimum range of the y-axis is set, and axes properties are configured.
% For details, look to the comments in MMplot.
% The third input to MMplot is an optional Matlab LineSpec. So, our first
% call plots MM_dirInt in blue with a linewidth of 2. The handles are
% stored in h.
...
% Plot MM_general on the same figure by providing the handles. Most
